...
patterns = {
    ...
    'LABEL': 'this place must be referred to as {id} so that we can find it when we need to',
    ...
    'RET': 'return back to the place that you promised to return back',
    ...
    'IF_SKIP': 'if it happens to be that {value} {id} {value} ignore the following {quantity} line/lines',
    'UNLESS_SKIP': 'unless it happens to be that {value} {id} {value} ignore the following {quantity} line/lines',
    'INPUT': 'wait right here until there is a useful ingress from the user',
    'WRITE_LN': 'write the line {value} to the console , skipping to the new line afterwards',
    'WRITE': 'write the line {value} to the console , do not skip to the new line',
...
with open(path, 'w') as read_file:
    read_file.write("// this header file is auto-generated using gen-parser-pattern.py script\n\n#pragma once\n")
